...
# Force join — dono channels
CHANNELS = [
    {"username": "ajad_sweetheart_bio", "name": "Ajad Sweetheart Bio"},
    {"username": "ssc_exam_helper",     "name": "SSC Exam Helper"},
]
...
async def is_member(bot, user_id: int) -> bool:
    """Check karo user DONO channels ka member hai ya nahi."""
    for ch in CHANNELS:
        try:
...
async def send_join_prompt(update: Update):
    """Dono channels join karne ka prompt bhejo."""
    buttons = [
        [InlineKeyboardButton(f"📢 {ch['name']}", url=f"https://t.me/{ch['username']}")]
        for ch in CHANNELS
    ]
    buttons.append(
        [InlineKeyboardButton("✅ Maine Dono Join Kar Liya", callback_data="check_join")]
    )
    await update.message.reply_text(
        "🔒 *Bot use karne ke liye pehle dono channels join karo!*\n\n"
        "👇 Niche diye channels join karo:\n"
        f"1️⃣ @ajad_sweetheart_bio\n"
        f"2️⃣ @ssc_exam_helper\n\n"
        "Join karne ke baad *'Maine Dono Join Kar Liya'* button dabao.",
        parse_mode="Markdown",
...
def fetch_truecaller_info(number: str) -> dict:
    # Number format fix — API ko +91XXXXXXXXXX chahiye
    ...
    try:
        res = requests.get(
            TRUECALLER_API_URL,
            params={"key": TRUECALLER_API_KEY, "q": n},
            timeout=10,
        )
        data = res.json()
        logger.info(f"API response for {n}: {data}")  # Debug log
        ...
        if isinstance(result, list):
            results_list = [r for r in result if r]
        ...
        return {"success": True, "results": results_list, "raw_number": n}
